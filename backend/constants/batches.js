...
export const BATCHES = [
  {
    ...
    name: 'Vibe Coding',
    code: 'VC',
    streamCodes: ['cv', 'vc', 'vibe', 'vibecoding'],
  ...
  },
  {
    ...
    name: 'AI',
    code: 'AI',
    streamCodes: ['ai', 'ml', 'artificial intelligence'],
  ...
  },
  {
    ...
    code: 'IC',
    streamCodes: ['ic', 'industry', 'industryconnect'],
...
export const resolveBatchId = (input) => {
  if (!input) return 'vibe-coding';
  const clean = input.toString().trim().toLowerCase();
  for (const b of BATCHES) {
    if (b.id === clean || b.code.toLowerCase() === clean || b.streamCodes.includes(clean)) {
      return b.id;
    }
  }
  if (clean.includes('cv') || clean.includes('vibe') || clean.includes('coding')) return 'vibe-coding';
  if (clean.includes('mk') || clean.includes('market')) return 'marketing';
  if (clean.includes('ai')) return 'ai';
  if (clean.includes('ic') || clean.includes('industry')) return 'industry-connect';
  return 'vibe-coding';
};
